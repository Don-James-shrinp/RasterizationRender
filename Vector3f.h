...
public:
    ...
    virtual Vector3f normalized();  //  返回归一化后的向量，而不对其本身作修改

    virtual void normalize();  //  对本身进行归一化

    virtual float dotMultiply(Vector3f other);  //  向量的点乘

    virtual Vector3f crossMultiply(Vector3f other);  // 向量叉乘,返回叉乘的结果向量
    ...
    virtual float getModulus();  //  获取向量的模长
